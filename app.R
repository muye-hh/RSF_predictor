...
# 加载模型数据（包含 rf_ranger, Train_expr, Train_surv, median_cut, model_features 等）
load("shiny_model_ranger.RData")
# 加载标准化参数（train_mean, train_sd）
load("standardization_params.RData")

# 模型对象名为 rf_ranger（不是 best_model）
model <- rf_ranger
# 从模型中提取基因列表
gene_list <- model$forest$independent.variable.names

# 提取模型基因对应的均值和标准差（train_mean 和 train_sd 是全部基因的）
gene_mean <- train_mean[gene_list]
gene_sd   <- train_sd[gene_list]
gene_sd[gene_sd == 0] <- 1e-6   # 防止除零

# ---------- 预测函数 ----------
predict_risk <- function(input_values, input_type = "TPM") {
  # 如果是 TPM，先 log2(TPM+1)
  if (input_type == "TPM") {
